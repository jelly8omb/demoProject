	...
	<div id="page-wrapper">
			...
			<ul id="table24">
				...
				<li id="content">
					<ul>
						...
						<li>
						<textarea rows="20" disabled="disabled"	style="background-color: #f0adca;" font-size="inherit;"><c:out value="${vo.content}" /></textarea>
						</li>
					</ul>
				</li>
			</ul>
			<div style="text-align: center; margin-bottom: 10px;">
				<c:if test="${user.userid==vo.writer }">
		...
		function execute(f){
			...
			<form action="comments" method="post">
				...
				<!-- 현재페이지 번호 전달 - 순서8) -->
				<input type="hidden" name="writer" class="input"
					value="${user.userid }">
				...
				<ul>
					...
					<li>
						<ul style="display: flex; flex-direction: column;">
							<li><textarea rows="5" cols="90" name="content"
					...
					<c:forEach var="cmt" items="${cmtlist}">
						<li>
							<ul class="crow">
								...
								<c:if test="${user.userid==cmt.writer }">
									<!-- session 에 저장된 user애트리뷰트의 id와 작성자의 id 가 같은면 보이기 -->
									<li><a href="javascript:executeCmt('2','${cmt.idx }')">삭제</a></li>
								</c:if>
							</ul>
						</li>
						<li><textarea class="cmtcontent" style="border:none;"><c:out value="${cmt.content }" /></textarea></li>
					</c:forEach>
				</ul>
			</form>
		</section>
